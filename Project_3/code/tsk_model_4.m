...
% TSK Model 4 -> 3 MFs, Polynomial output
...
fprintf('TSK Model 4 -> 3 MFs, Singleton output\n');
...
% Generate FIS again with 3 MFs and singleton output
opt = genfisOptions('GridPartition');
...
sgtitle('TSK model 4: MFs before Training')
saveas(gcf, 'TSK_model_4_imgs/4_MFs_before_Training.png')
...
sgtitle('TSK model 4: MFs after Training')
saveas(gcf, 'TSK_model_4_imgs/4_MFs_after_Training.png')
...
title('Representative Run (Model 4): Learning Curve');
grid on;
saveas(gcf, 'TSK_model_4_imgs/4_Learning_Curve.png');

% --- Plot prediction errors for representative run ---
figure;
plot(error_raw)
title('TSK model 4: Prediction Errors (Raw Data)')
xlabel('Sample Index')   
ylabel('Error')          
saveas(gcf,'TSK_model_4_imgs/4_Pred_Errors.png')
