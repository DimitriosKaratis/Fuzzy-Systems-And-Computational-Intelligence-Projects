...
% TSK Model 3 -> 2 MFs, Polynomial output
...
fprintf('TSK Model 3 -> 2 MFs, Polynomial output\n');
...
for run = 1:numRuns
    ...
    % Generate initial FIS using grid partitioning (2 MFs per input)
    opt = genfisOptions('GridPartition');
    opt.NumMembershipFunctions = [2 2 2 2 2];                   % 2 MFs per input
    opt.InputMembershipFunctionType = ["gbellmf" "gbellmf" "gbellmf" "gbellmf" "gbellmf"];
    opt.OutputMembershipFunctionType = 'linear';               % Polynomial output
...
% Generate FIS again with 2 MFs and polynomial output
opt = genfisOptions('GridPartition');
...
sgtitle('TSK model 3: MFs before Training')
saveas(gcf, 'TSK_model_3_imgs/3_MFs_before_Training.png')
...
sgtitle('TSK model 3: MFs after Training')
saveas(gcf, 'TSK_model_3_imgs/3_MFs_after_Training.png')
...
title('Representative Run (Model 3): Learning Curve');
grid on;
saveas(gcf, 'TSK_model_3_imgs/3_Learning_Curve.png');

% --- Plot prediction errors for representative run ---
figure;
plot(error_raw)
title('TSK model 3: Prediction Errors (Raw Data)')
xlabel('Sample Index')   
ylabel('Error')          
saveas(gcf,'TSK_model_3_imgs/3_Pred_Errors.png')
