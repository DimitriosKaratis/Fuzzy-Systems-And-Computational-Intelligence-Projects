...
% TSK Model 1 -> class-independent + radius = 0.2 ("small")
...
fprintf('--- Class-Independent TSK Classification --- Model 1 ---\n');
...
for run = 1:numRuns
    ...
    %% Build FIS using Subtractive Clustering (class-independent)
    radius = 0.2;  % Cluster radius (controls number of clusters/rules)
    options = genfisOptions('SubtractiveClustering','ClusterInfluenceRange',radius);
    fis = genfis(trainingData(:,1:3), trainingData(:,4), options);

    % Force the output membership functions to be "constant" (singleton)
    for i = 1:length(fis.output.mf)
        fis.output.mf(i).type = 'constant';
    end

    %% Train FIS using ANFIS (hybrid learning method)
    ...
    %% Evaluate the trained FIS on the test set
    y_pred = evalfis(chkFIS, testData(:,1:3));
    
    % Round continuous outputs to nearest class label
...
%% Plotting only for representative run 
...
for i=1:3
    ...
    plot(x,mf); title(['Input ' num2str(i) ' MFs (Before)']);
end
sgtitle('Membership Functions Before Training');
saveas(gcf, fullfile('TSK_model_1_imgs','1_MFsBeforeTraining.png'));
...
for i=1:3
    ...
    plot(x,mf); title(['Input ' num2str(i) ' MFs (After)']);
end
sgtitle('Membership Functions After Training');
saveas(gcf, fullfile('TSK_model_1_imgs','1_MFsAfterTraining.png'));

% Confusion matrix heatmap
figure;
confusionchart(repRun.y_true, repRun.y_pred);
title('Confusion Matrix (Representative Run)');
saveas(gcf, fullfile('TSK_model_1_imgs','1_ConfusionMatrix.png'));
...
title('Prediction Errors (Representative Run)'); grid on;
saveas(gcf, fullfile('TSK_model_1_imgs','1_PredictionErrors.png'));
...
% Mark the epoch with minimum validation error
[~, minValIdx] = min(repRun.chkError);
plot(minValIdx, repRun.chkError(minValIdx), 'go', ...
    'MarkerSize',8, 'LineWidth',2, 'MarkerFaceColor','g');

xlabel('Epochs'); 
ylabel('Error');
legend('Training','Validation','Best Validation (chkFIS)','Location','best'); 
grid on;
title('Learning Curve (Representative Run)');
saveas(gcf, fullfile('TSK_model_1_imgs','1_LearningCurve.png'));

% Display radius and number of rules in representative run
